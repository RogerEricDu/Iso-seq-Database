    ...
    <div class="filter-container">
      <el-form :inline="true" :model="query">
        ...
        <div class="button-wrapper">
          <el-button type="primary" @click="filterGene">Query Gene Annotation Information</el-button>
...
<script>
import { getGeneInfo } from '@/api/geneInfo'
...
export default {
  data() {
    return {
      ...
      columns: [
        ...
        { label: 'Chromosome', prop: 'chromosome', width: 120 },
        { label: 'Start', prop: 'start', width: 140 },
        { label: 'End', prop: 'end', width: 140 },
        { label: 'Strand', prop: 'strand', width: 120 },
        { label: 'GeneType', prop: 'geneType', width: 200 },
        { label: 'ExonNumber', prop: 'exonNumber', width: 200 },
        { label: 'AssociatedTranscript', prop: 'associatedTranscript', width: 200 }
      ]
    }
  },
  computed: {
  ...
  methods: {
    ...
    // ⭐ 点击查询
    filterGene() {
      const keyword = String(this.query.gene || '').trim() // 去掉前后空格并转化为字符串

      if (!keyword) {
        this.filteredList = this.list // 默认显示全部（如果keyword为空则返回true）
      } else {
        this.filteredList = this.list.filter(item =>
          String(item.geneName || '').includes(keyword) || // “或者”
          String(item.geneId || '').includes(keyword)
        )
      }
      this.currentPage = 1
    },
